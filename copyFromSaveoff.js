...
if (fs.existsSync(path.resolve(destinationDir, './www'))) {
  rimraf('../WeVoteCordova/www', () => {
    ...
    fs.mkdir(path.resolve(destinationDir, './www'), (err) => {
      ...
      const filesToCopy = ['./config.xml','./www/index.html', './.gitignore', './.npmignore', './buildSymLinks.js', './copyFromSaveoff.js', './package.json', 'package-lock.json','README.md'];
